...
export function isSentMessageTitle(tit: unknown): tit is SentMessageTitle {
  if (typeof tit != "string") return false;

  if (!["chat", "update-peer", "canvas-definition", "cursor-move", "frame-update"].includes(tit)) return false;
...
export function isMessageObject(
  ...
  // we will assume at this point that the data is valid. This should change, but is hard and expensive. So far we
